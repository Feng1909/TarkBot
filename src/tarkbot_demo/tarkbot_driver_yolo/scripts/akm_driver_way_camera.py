...
def side_flag_callback(msg):
	...
	if msg.data == [] or msg.data[0].frame_id == "Green_light":
		...
		if msg.data[0].ptx > 525 and msg.data[0].pty < 80:
			stop_flag = 1
			stop_temp = 0
		...
		if start_flag == 1:
			if tarkbot_driver_way == 0:  
				...
				if side == 2:
					...
					last_side = side
					ctrldata_pub = ctrl_data(tarkbot_driver_way, side)
					driver_mode_pub.publish(ctrldata_pub)
					print("Inner_ring")
					side = 0
					side_flag = 0
					time.sleep(2)
				...
				if stop_flag == 1:
					...
					if stop_back_run == 1:
						...
						cmdvel_pub.publish(carmove)
						time.sleep(3)
						ctrldata_pub = ctrl_data(last_tarkbot_driver_way, 2)
						driver_mode_pub.publish(ctrldata_pub)
						stop_back_run = 0
						stop_flag = 0
						print("Inner_ring")
