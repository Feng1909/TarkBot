...
namespace opencv_apps
{
  ...
  public:
    ...
    public:
      ...
      virtual void clamp(RGBColorFilterConfig &config, const RGBColorFilterConfig &max, const RGBColorFilterConfig &min) const override
      {
      ...
      virtual void calcLevel(uint32_t &comb_level, const RGBColorFilterConfig &config1, const RGBColorFilterConfig &config2) const override
      {
        if (config1.*field != config2.*field)
          comb_level |= level;
      }

      virtual void fromServer(const ros::NodeHandle &nh, RGBColorFilterConfig &config) const override
      {
        nh.getParam(name, config.*field);
      }

      virtual void toServer(const ros::NodeHandle &nh, const RGBColorFilterConfig &config) const override
      {
        nh.setParam(name, config.*field);
      }

      virtual bool fromMessage(const dynamic_reconfigure::Config &msg, RGBColorFilterConfig &config) const override
      {
        return dynamic_reconfigure::ConfigTools::getParameter(msg, name, config.*field);
      }

      virtual void toMessage(dynamic_reconfigure::Config &msg, const RGBColorFilterConfig &config) const override
      {
        dynamic_reconfigure::ConfigTools::appendParameter(msg, name, config.*field);
      }

      virtual void getValue(const RGBColorFilterConfig &config, boost::any &val) const override
      {
        val = config.*field;
      }
    };
    ...
    public:
      ...
      virtual void updateParams(boost::any &cfg, RGBColorFilterConfig &top) const override
      {
        PT* config = boost::any_cast<PT*>(cfg);
